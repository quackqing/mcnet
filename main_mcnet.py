...
def pre_block(xm, mcSeq, pool_size):
    ...
    print('进入pre-block模块')
    base = xm
    xm0 = Conv2D(32, (1, 3), padding='same', name=mcSeq + "_pre_block model_conv00", kernel_initializer='glorot_normal', data_format='channels_first')(base)
    xm0 = MaxPooling2D(pool_size=pool_size, strides=pool_size, padding='valid', data_format='channels_first')(xm0)

    xm1 = Conv2D(32, (1, 3), padding='same', name=mcSeq + "_pre_block model_conv01", kernel_initializer='glorot_normal', data_format='channels_first')(base)

    xm = kb.concatenate([xm0, xm1], axis=0)
    # concat(xm0, xm1)
    return xm
...
def m_block(xm, mcSeq):
    ...
    xm0 = Conv2D(32, (3, 1), padding='same', name=mcSeq + "_m_block pool", kernel_initializer='glorot_normal', data_format='channels_first')(base_xm)
    xm1 = Conv2D(32, (1, 3), padding='same', name=mcSeq + "_m_block model_conv01", kernel_initializer='glorot_normal', data_format='channels_first')(base_xm)
    xm2 = Conv2D(32, (1, 1), padding='same', name=mcSeq + "_m_block model_conv02", kernel_initializer='glorot_normal', data_format='channels_first')(base_xm)

    xm = kb.concatenate([xm0, xm1], axis=0)
    xm = kb.concatenate([xm, xm2], axis=0)
    # concat(xm0, xm1, xm2)
    return xm
...
def m_block_p(xm,mcSeq, pool_size):
    ...
    xm0 = MaxPooling2D(pool_size=pool_size, name=mcSeq+'_m_block_p_pool', kernel_initializer='glorot_normal', data_format='channels_first')(xm0)
    xm1 = Conv2D(32, (1, 3), padding='same', name=mcSeq + "_pre_block_p_model_conv01", kernel_initializer='glorot_normal', data_format='channels_first')(base_xm)
...
# conv
xm = Conv2D(64, (3, 7), padding='same', name='conv0', kernel_initializer='glorot_normal', data_format='channels_first')(xm)


# pool
xm = MaxPooling2D(pool_size=(1, 3), name='pool0', kernel_initializer='glorot_normal', data_format='channels_first')(xm)


# keras.backend.concatenate

# pre-block
mcSeq = 'mc_net'    # 可以自行定义，每一层网络的名字。。。
pool_size = (1, 3)
xm = pre_block(xm, mcSeq, pool_size)
xm_tmp1 = MaxPooling2D(pool_size=(1, 3), name='pool0', kernel_initializer='glorot_normal', data_format='channels_first')(xm)

# pool
xm = MaxPooling2D(pool_size=(1, 3), name='pool1', kernel_initializer='glorot_normal', data_format='channels_first')(xm)

# M-block-p
xm = m_block_p(xm, mcSeq, pool_size)

# add
xm = kb.concatenate([xm, xm_tmp1], axis=0)
xm_tmp2 = xm
...
# add
xm = kb.concatenate([xm, xm_tmp2], axis=0)
xm_tmp3 = xm
xm_tmp3_pool = MaxPooling2D(pool_size=(1, 3), name='pool0', kernel_initializer='glorot_normal', data_format='channels_first')(xm_tmp3)
...
# add
xm = kb.concatenate([xm, xm_tmp3_pool], axis=0)
xm_tmp4 = xm
# M-block
xm = m_block(xm, mcSeq)

# add
xm = kb.concatenate([xm, xm_tmp4], axis=0)
xm_tmp5 = xm
xm_tmp5_pool = MaxPooling2D(pool_size=(1, 3), name='pool0', kernel_initializer='glorot_normal', data_format='channels_first')(xm_tmp5)
# M-block-p
xm = m_block_p(xm, mcSeq, pool_size)
# add
xm = kb.concatenate([xm, xm_tmp5_pool], axis=0)
xm_tmp6 = xm
...
# concat
xm = kb.concatenate([xm, xm_tmp7], axis=0)

# avg-pool
xm = AveragePooling2D(pool_size=(1, 3), name='pool0', kernel_initializer='glorot_normal', data_format='channels_first')(xm)
